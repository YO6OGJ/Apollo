...
//  power_all_disable(); //Needs to be disabled to enable USB to work
...
{
  volatile char dinnye[] = "dummy"; //Not sure why this is neded. Wasted days debugging PTT crashing the controller after a couple of calls.
  //May be because of some sort of compiler optimization
  //When Kernel debug is enabled everything works fine. 
  //So if no other problem is found. This hack is here to stay.
  ASSERT(queue != NULL && spinlock != NULL);
  queue_append(queue, current_task);
