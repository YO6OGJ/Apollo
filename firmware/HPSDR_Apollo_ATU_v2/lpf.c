/* Apollo ATU firmware
	 ...
	 Modifed 2019 by Csaba Nagy, YO6PVH
...
void disable_lpf ()
{
  if (lpf_enabled) {
    ...
    lock_acquire (&lpf_lock);
	if (current_band != LPF_BYPASS)
	{
		new_band = LPF_BYPASS; //If filter is disabled we have to switch to bypass
		condition_signal (&lpf_cond);
	}
    lock_release (&lpf_lock);
  }
...
//Get frequency for filter setting 
uint32_t lpf_get_FreqForBand(uint8_t band)
{
	if (band >= LPF_BYPASS)
		return lpf_cutoff[LPF_BYPASS];
	return lpf_cutoff[band];
}
